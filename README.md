...
4. Record an activity. Three ways it uploads:
   ...
   The app has two tabs: **Ajustes** (server + key, test, self-update) and **Historial**.
5. The **Historial** tab shows each upload's status (uploading / uploaded / error) with a **Ver en
   Endurain ↗** link that opens the activity at `<server>/activity/<id>`.
6. **Self-update:** in **Ajustes → Buscar actualizaciones**, the app checks GitHub for a newer signed
   release and can download + install it in place (grant "install unknown apps" once when prompted).
